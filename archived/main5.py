...
class Bootstrapper:
    ...
    def train_model(self):
        ...
        for letter in valid_classes:
            ...
            cloud[:, 5] = np.clip(np.round(cloud[:, 5]), 0, 1) 
...
class AbeyanceSwarm:
    ...
    def _agent_loop(self):
        while self.running:
            ...
            if self.max_lifespan_var:
                max_life = self.max_lifespan_var.get()
                stale_notes = [note for note, start_time in self.active_ai_notes.items() if now - start_time > max_life]
                
                for note in stale_notes:
                    if note in self.active_ai_notes:
                        del self.active_ai_notes[note]
                    if self.midi_out:
                        try:
                            off_msg = mido.Message('note_off', note=note, velocity=0)
                            self.midi_out.send(off_msg)
                            self.feedback_filter.log_sent(off_msg, now)
                            if self.gui_root:
                                self.gui_root.after(0, lambda n=note: self.ai_roll.set_note(n, False))
                        except Exception as e:
                            print(f"GC Error: {e}")

            if self.forge.is_trained and self.record_state == "IDLE":
                for agent in self.agents.values():
                    agent.tick()
                    
                if self.gui_root:
                    for letter, agent in self.agents.items():
                        bar = self.energy_bars.get(letter)
                        if bar: self.gui_root.after(0, lambda b=bar, e=agent.energy: b.config(value=e*100))
    ...
    def _agent_play_callback(self, msg):
        ...
        if self.midi_out:
            if msg.type == 'note_on' and msg.velocity > 0:
                self.active_ai_notes[msg.note] = now 
                
                limit = self.max_poly_var.get()
                while len(self.active_ai_notes) > limit:
                    stolen_note = min(self.active_ai_notes, key=self.active_ai_notes.get) 
    ...
    def _init_gui(self):
        ...
        self.gui_root.geometry("800x800")
        ...
        ttk.Label(poly_frame, text="Max AI Polyphony:", font=("", 9)).pack(side="left", padx=2)
        self.max_poly_var = tk.IntVar(value=16)
        ttk.Spinbox(poly_frame, from_=2, to=64, increment=2, textvariable=self.max_poly_var, width=3).pack(side="left")

        ttk.Label(poly_frame, text="| Max Note Life(ms):", font=("", 9)).pack(side="left", padx=2)
        self.max_lifespan_var = tk.IntVar(value=8000)
        ttk.Spinbox(poly_frame, from_=1000, to=60000, increment=500, textvariable=self.max_lifespan_var, width=5).pack(side="left")

        ttk.Label(poly_frame, text="| Phrase Pause(ms):", font=("", 9)).pack(side="left", padx=2)
